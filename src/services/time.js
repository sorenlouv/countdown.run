...
export function getTimeRemaining(countdown) {
  const [hour1, hour2, min1, min2, sec1, sec2] = padStart(
    countdown.time,
    6,
    '0'
  ).split('');
  const countdownDuration =
    (parseInt(hour1 + hour2, 10) * 3600 +
      parseInt(min1 + min2, 10) * 60 +
      parseInt(sec1 + sec2, 10)) *
    1000;
  ...
  const timeRemaining =
    deadlineAt - (countdown.pausedAt || Date.now()) + countdown.pausedDuruation;
  if (timeRemaining < 0) {
    return 0;
  }
